...
class UpdateStubDslTest {
    ...
    @Test fun `check basic functionality`() = runBlocking{
        ...
        assert(dynResult.value == 1)
        assert(started.value)
        assert(updatePerformed.value)
    ...
    @Test fun `update parent from child stub`() = runBlocking {
        ...
        delay(100)
        assert(updatedData.value == Data(1))
        parent.stop()
        delay(500)
    }

    @Test fun `update stub as child of standard stub should integrate properly`() {
        data class Data(val x: Int)
        ...
        assert(stub.stubs[Child::class] is UpdateStub)
        assert(stub.stubs[Child::class]!!.parent == stub)
    }
}