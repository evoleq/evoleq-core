...
class InputStubConfiguration<I, Data> : StubConfiguration<Data>() {

    private class InputStubConfigurationException(override val message: String?) : Exception(message)

    private var onStart: suspend CoroutineScope.(Data)->Data = {data -> data}
    ...
    override fun configure(): InputStub<I, Data> = with(object : InputStub<I, Data>(){
        override val id: ID
    ...
    @EvoleqDsl
    open fun <J, E> InputStubConfiguration<I, Data>.inputChild(stub: InputStub<J, E>) { this@InputStubConfiguration.inputChild(stub.id, stub.configuration().second) }


}

@EvoleqDsl
fun <I, Data> inputStub(configuration: InputStubConfiguration<I, Data>.()->Unit): InputStub<I, Data> = with(InputStubConfiguration<I, Data>()) {
    configuration()
    configure()
}//= configure(configuration) as InputStub<I, Data>

@EvoleqDsl
fun <J, E> StubConfiguration<*>.inputChild(childId: ID,configuration: InputStubConfiguration<J, E>.()->Unit) {
    childConfigurations[childId] = Pair(InputStubConfiguration<J, E>(),configuration as StubConfiguration<*>.() -> Unit)
}

@EvoleqDsl
fun <J, E> StubConfiguration<*>.inputChild(stub: InputStub<J, E>) { inputChild(stub.id, stub.configuration().second) }


@EvoleqDsl
fun <I, Data> InputStub<I, Data>.configuration(): Pair<out StubConfiguration<*>,InputStubConfiguration<I, Data>.()->Unit> =
